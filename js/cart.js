...
//添加进购物车
function CartAdd2(obj, webpath, linktype, linkurl, goods_id, goods_quantity) {
	if ($("#goods_id").val() == "" || $("#goods_quantity").val() == "") {
		return false;
	}
	$.ajax({
...
//添加进购物车
function CartAddKit(obj, webpath, linktype, linkurl, goods_id, goods_quantity) {
	if (obj.checked == true) {
		if ($("#goods_id").val() == "" || $("#goods_quantity").val() == "") {
			return false;
		}
...
//添加进购物车
function CartAdd3(obj, webpath, linktype, linkurl, goods_id, goods_quantity) {
	if ($("#goods_id").val() == "" || $("#goods_quantity").val() == "") {
		return false;
	}
	$.ajax({
...
//添加送货单
function POAdd(obj, webpath, linktype, linkurl, goods_id, goods_quantity) {
	if ($("#goods_id").val() == "" || $("#goods_quantity").val() == "") {
		return false;
	}
	$.ajax({
		type: "post",
		url: webpath + "tools/submit_ajax.ashx?action=po_goods_add",
		data: {
			"goods_id": goods_id,
			"goods_quantity": goods_quantity
		},
			...
			if (data.status == 1) {
				if (linktype == 1) {
					...
					var HintHtml = '<div id="cart_info_hint" class="msg_tips cart_info">'
						...
						+ '<div class="msg">'
						+ '<strong>商品成功添加到购物车！</strong>'
						+ '<p>送货单共有<b><font color=red>' + data.quantity + '</font></b>种商品</p>'
						+ '<a class="btn btn-success" title="去建立送货单" href="' + linkurl + '">送货单</a>&nbsp;&nbsp;'
						+ '<a title="再逛逛" href="javascript:;" onclick="HintRemove(\'cart_info_hint\');">再逛逛</a>'
						+ '<a class="close" title="关闭" href="javascript:;" onclick="HintRemove(\'cart_info_hint\');"><span>关闭</span></a>'
						+ '</div>'
						+ '</div>'
